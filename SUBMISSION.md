...
CodeRouter AI is a **cost-aware AI coding assistant** that classifies the complexity of every query before deciding how to answer it.
...
User Query
 ...
 using local model
 (always free, no credits used)
      ↓
 Score ≤ 3          Score > 3
    ↓                    ↓
🟢 Local Model     🔴 Remote Model
Ollama             Fireworks AI
qwen2.5-coder      GLM 5.2
FREE               Credits used sparingly
      ↓                    ↓
      Response + Cost Breakdown
```

The classifier itself runs **entirely on the local model** — meaning even the routing decision costs zero credits. Only tasks that genuinely require deeper reasoning are escalated to Fireworks AI.
...
**Intelligent Routing**
Queries are scored 1–5 for complexity using a tuned classifier prompt with concrete examples. Scores 1–3 stay local. Scores 4–5 go remote. The threshold is configurable via environment variable.
...
In testing across a wide range of developer queries, CodeRouter AI routes the majority of typical questions locally — eliminating those credit costs entirely while reserving remote model calls for tasks that genuinely benefit from more powerful inference.

**Complexity distribution from real testing:**

| Complexity | Type | Routing | Example |
|---|---|---|---|
| 1 | Trivial | 🟢 Local | "what is a variable" |
| 2 | Simple | 🟢 Local | "write a hello world" |
| 3 | Moderate | 🟢 Local | "write a binary search" |
| 4 | Complex | 🔴 Remote | "build a REST API with JWT auth" |
| 5 | Expert | 🔴 Remote | "design a microservices architecture" |
...
| Local Model | Ollama + qwen2.5-coder:1.5b | Free local inference |
| Remote Model | Fireworks AI + GLM 5.2 | Powerful remote inference |
...
**Node 1 — Classifier**
Takes the raw user query and scores its complexity from 1 to 5 using the local Ollama model. The prompt includes concrete examples for each score level to ensure consistent, deterministic output (temperature: 0). This node never touches Fireworks AI.

**Node 2 — Router**
Reads the complexity score and the configurable threshold from `.env`. Makes a binary decision: local or remote. Simple, fast, stateless.

**Node 3 — Inference**
Calls the appropriate model. Returns the response along with token count and cost metadata. On remote failure, returns a descriptive error message rather than silently falling back to an inferior local response.
